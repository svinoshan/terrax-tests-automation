...
export class FarmerListPage extends BasePage {
  ...
  async expectLoaded(): Promise<void> {
    ...
    await expect(
      this.page.getByRole("columnheader", { name: /Cb ref id/i }),
    ).toBeVisible();
    await expect(
      this.page.getByRole("columnheader", { name: /^Name$/i }),
    ).toBeVisible();
    await expect(
      this.page.getByRole("columnheader", { name: /Gender/i }),
    ).toBeVisible();
    await expect(
      this.page.getByRole("columnheader", { name: /Main unit/i }),
    ).toBeVisible();
    await expect(
      this.page.getByRole("columnheader", { name: /Sub unit/i }),
    ).toBeVisible();
    await expect(
      this.page.getByRole("columnheader", { name: /City/i }),
    ).toBeVisible();
    await expect(
  ...
  async getFarmerRow(farmerCodeOrName: string): Promise<Locator> {
    const row = this.page
      .getByRole("row")
      .filter({ hasText: farmerCodeOrName })
      .first();
  ...
  async expectFarmerCreatedStatus(farmerCodeOrName: string): Promise<void> {
    const row = await this.getFarmerRow(farmerCodeOrName);

    await expect(row).toContainText(/Active|GPS Not Completed/i, {
      timeout: 10000,
    });
  }
