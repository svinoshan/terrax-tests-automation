...
test.describe("@regression Update Purchase", () => {
  test("user can update unauthorized purchase note", async ({
    authenticatedUser,
    farmerListPage,
    farmerProfilePage,
    purchaseListPage,
    ...
    const updatedNote = `${purchaseData.note} updated`;
    const updatedPurchaseQty = "15";
    const updatedUnitPrice = "150";

    // Arrange: create active farmer.
    ...
    // Arrange: create unauthorized purchase using Save only.
    ...
    // Act: edit unauthorized purchase.
    ...
    await createPurchasePage.update();
    ...
    // Assert: purchase still listed and remains unauthorized.
    await purchaseListPage.expectLoaded();
