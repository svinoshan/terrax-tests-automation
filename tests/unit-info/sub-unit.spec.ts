...
test.describe('@regression Sub Unit', () => {
  test('user can create and update sub unit', async ({
    ...
    await subUnitPage.expectRowContains(subUnit.subUnitCode, subUnit.description);
    ...
    await subUnitPage.search(subUnit.subUnitCode);
    await subUnitPage.expectRowContains(subUnit.subUnitCode, updatedSubUnitDescription);
  });
});
